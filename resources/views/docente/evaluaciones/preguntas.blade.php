...
@section('title', 'Preguntas de la Evaluacion')

@section('content')
<div class="max-w-5xl mx-auto space-y-6">
    <div class="flex items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Preguntas de la evaluacion</h1>
            <p class="text-gray-500">{{ $evaluacion->titulo }} ({{ $evaluacion->preguntas->count() }} preguntas)</p>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form method="POST" action="{{ route('docente.evaluaciones.preguntas.agregar', $evaluacion->id) }}" class="p-6 space-y-4">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="respuesta_correcta" name="respuesta_correcta" required class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-purple-500" placeholder="Ej: B o Verdadero">
                </div>
        ...
        <div class="divide-y divide-gray-100">
                ...
                <div class="p-6 flex items-start justify-between gap-4">
                    <div class="space-y-2">
                        ...
                        @if(!empty($pregunta->respuesta_correcta))
                            <div class="text-sm text-green-700">Respuesta: {{ is_array($pregunta->respuesta_correcta) ? implode(', ', $pregunta->respuesta_correcta) : $pregunta->respuesta_correcta }}</div>
                        @endif
                    </div>

                    <form method="POST" action="{{ route('docente.evaluaciones.preguntas.eliminar', [$evaluacion->id, $pregunta->id]) }}" onsubmit="return confirm('Eliminar esta pregunta?')">
                        @csrf
                        @method('DELETE')
            ...
            @empty
                <div class="p-10 text-center text-gray-500">
                    Aun no hay preguntas para esta evaluacion.
                </div>
            @endforelse
        </div>
    </div>
