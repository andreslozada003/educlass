...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-6 text-white">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-green-100 text-sm">Precision</p>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Puntuacion</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($intentos as $intento)
                        @php
                            $juego = $intento->juego;
                            $puntajeTotal = $juego ? $juego->preguntasActivas->sum('puntaje') : 0;
                            if ($puntajeTotal <= 0) {
                                $puntajeTotal = optional($juego)->puntaje_base ?: 0;
                            }
                            $precision = $intento->porcentaje_aciertos ?? 0;
                            $fechaIntento = $intento->fecha_intento ?? $intento->created_at;
                        @endphp
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="p-2 bg-purple-100 rounded-lg mr-3">
                                        <span class="text-xl">{{ $juego->tipo_icono ?? 'J' }}</span>
                                    </div>
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">{{ $juego->titulo ?? 'Juego no disponible' }}</div>
                                        <div class="text-sm text-gray-500">{{ ucfirst($juego->tipo ?? 'juego') }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ data_get($juego, 'tema.asignatura.nombre', 'Sin asignatura') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <span class="font-medium">{{ $intento->puntaje_obtenido ?? 0 }}</span>
                                    <span class="text-gray-400 mx-1">/</span>
                                    <span class="text-gray-500">{{ $puntajeTotal }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $fechaIntento ? $fechaIntento->diffForHumans() : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                @if($juego)
                                    <a href="{{ route('estudiante.juegos.jugar', $juego->id) }}" class="text-purple-600 hover:text-purple-900">
                                        Jugar de Nuevo
                                    </a>
                                @else
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-gray-500">
                                ...
                                <p class="text-lg">Aun no has jugado</p>
